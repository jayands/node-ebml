module.exports = class Tools {
  ...
  static readVint(buffer, start = 0) {
    ...
    for (let i = 1; i < length; i += 1) {
      if (i === 7) {
        if (value >= 2 ** (53 - 8) && buffer[start + 7] > 0) {
          return {
            length,
            value: -1,
          };
  ...
  /**
     ...
     * @param {Number} [start=0] the starting position (defaults to 0)
     * @param {Number} [end=buff.byteLength] the ending position (defaults to the whole buffer)
  ...
  /**
     * Reads a floating point decimal from the specified buffer
     *
  ...
  /**
     * Read the appropriate data from a tag
     *
     * @param {*} tagObj The the tag that holds the metadata to be read
     * @param {Uint8Array} data The data to read
     * @returns {*} The appropriately parsed object
     */
  static readDataFromTag(tagObj, data) {
    const tmpTag = { ...tagObj, data };
    switch (tagObj.type) {
      case 'u':
        tmpTag.value = Tools.readUnsigned(data);
        break;
      case 'f':
        tmpTag.value = Tools.readFloat(data);
        break;
      case 'i':
        tmpTag.value = Tools.readSigned(data);
        break;
      case 's':
        tmpTag.value = String.fromCharCode.apply(null, data);
        break;
      case '8':
        tmpTag.value = Tools.readUtf8(data);
        break;
      default:
        break;
    }

    if (tmpTag.name === 'SimpleBlock' || tmpTag.name === 'Block') {
      let p = 0;
      const track = Tools.readVint(data, p);
      p += track.length;
      tmpTag.track = track.value;
      tmpTag.value = Tools.readSigned(data.subarray(p, p + 2));
      p += 2;
      if (tmpTag.name === 'SimpleBlock') {
        tmpTag.keyframe = Boolean(data[track.length + 2] & 0x80);
        tmpTag.discardable = Boolean(data[track.length + 2] & 0x01);
      }
      p += 1;
      tmpTag.payload = data.subarray(p);
    }
