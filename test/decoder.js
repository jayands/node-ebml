...
describe('embl', () => {
  describe('Decoder', () => {
    it('should wait for more data if a tag is longer than the buffer', () => {
      ...
      assert.equal(STATE_TAG, decoder.state);
      assert.equal(2, decoder.buffer.length);
      assert.equal(0, decoder.cursor);
    });
    ...
    it('should clear the buffer after a full tag is written in one chunk', () => {
      ...
      assert.equal(STATE_TAG, decoder.state);
      assert.equal(0, decoder.buffer.length);
      assert.equal(0, decoder.cursor);
    });
    ...
    it('should increment the cursor on each step', () => {
      ...
      assert.equal(STATE_TAG, decoder.state);
      assert.equal(1, decoder.buffer.length);
      assert.equal(0, decoder.cursor);

      decoder.write(Buffer.from([0x86])); // 5

      assert.equal(STATE_SIZE, decoder.state);
      assert.equal(2, decoder.buffer.length);
      assert.equal(2, decoder.cursor);

      decoder.write(Buffer.from([0x81])); // 6 & 7

      assert.equal(STATE_CONTENT, decoder.state);
      assert.equal(3, decoder.buffer.length);
      assert.equal(3, decoder.cursor);
