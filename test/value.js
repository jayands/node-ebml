            ...
            decoder.on('data', (chunk) => {
                if (chunk[0] === 'tag' && chunk[1].name === 'SimpleBlock') {
                    if (chunk[1].value > 0) {
                        ...
                        assert.strictEqual(chunk[1].payload.byteLength, 43, 'payload length')
                        assert.strictEqual(chunk[1].track, 1, 'track')
                        assert.strictEqual(chunk[1].value, 96, 'value (timestamp)')
                        done()
                    }
                }
            })
            ...
            decoder.on('data', (chunk) => {
                if (chunk[0] === 'tag' && chunk[1].name === 'SimpleBlock') {
                    if (chunk[1].value > 0) {
                        ...
                        assert.strictEqual(chunk[1].discardable, false, 'discardable')
                        done()
                    }
                }
            })
